...
export abstract class AgentAuth extends MindConnectBase
  ...
  public async OnBoard(): Promise<OnboardingStatus.StatusEnum> {
    ...
    log(
      `Onboarding - Headers: ${JSON.stringify(
        headers
      )} Url: ${url} Profile: ${this.GetProfile()}`
    );
    try {
      let body: object = {};
  ...
  private PushKey() {
    ...
    if (!_.some(this._configuration.recovery, this._configuration.response)) {
      this._configuration.recovery.push(
        Object.assign({}, this._configuration.response)
      );
    }
  ...
  private async RotateKey(): Promise<boolean> {
    ...
    log(
      `Rotating Key - Headers: ${JSON.stringify(
        headers
      )} Url: ${url} Profile: ${this.GetProfile()}`
    );
  ...
  private CreateClientAssertion(expiration: number = 3600): URLSearchParams {
    if (!this._configuration.response) {
      throw new Error(
        "the device was not onborded or the response was deleted"
      );
    }
  ...
  private async AquireToken(): Promise<boolean> {
    ...
    log(
      `Acquire Token Headers ${JSON.stringify(
        headers
      )} Url: ${url} Body: ${body.toString()}`
    );
  ...
  private isSecretExpired(): boolean {
    ...
    const now = Math.floor(Date.now() / 1000);
    const secondsLeft =
      this._configuration.response.client_secret_expires_at - now;
  ...
  private async ValidateToken(): Promise<boolean> {
    if (!this._accessToken)
      throw new Error(
        "The token needs to be acquired first before validation."
      );
  ...
  public async RenewToken(): Promise<boolean> {
    ...
    if (
      this._configuration.response.client_secret_expires_at - 25 * 3600 <=
      now
    ) {
  ...
  public SetupAgentCertificate(privateCert: string | Buffer) {
    if (this.GetProfile() !== "RSA_3072") {
      throw new Error(
        "The certificates are required only for RSA_3072 configuration!"
      );
    }

    if (!privateCert) {
      throw new Error(
        "you need to create the certificate for the agent and provide the path to the agent"
      );
    }

    this._privateCert = privateCert.toString();
    this._publicJwk = rsaPemToJwk(
      this._privateCert,
      { kid: "mindconnect-key-1" },
      "public"
    );
    log(this._publicJwk);
  }
  ...
  constructor(
    ...
    super();
    log(
      `constructor called with parameters: configuration: ${JSON.stringify(
        configuration
      )} path: ${basePath}`
    );
    ...
    if (typeof basePath === "string") {
      ...
      throw new Error(
        "you have to specify either a directory or configuration storage"
      );
    }
