...
export abstract class CredentialAuth extends MindConnectBase
  ...
  private async ValidateToken(): Promise<boolean> {
    ...
    if (!publicKey) {
      throw new Error(
        `Token validation error, can't find certificate for ${tokenkey}`
      );
    }
  ...
  constructor(
    ...
    if (!isUrl(_gateway)) {
      throw new Error(
        "the gateway must be an URL (e.g. https://gateway.eu1.mindsphere.io"
      );
    }
