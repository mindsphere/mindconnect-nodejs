...
export default (program: CommanderStatic) => {
  program
    ...
    .option(
      "-i, --timeseries",
      `use ${yellow("(deprecated)")} standard timeseries upload`
    )
    ...
    .action(options => {
      (async () => {
        try {
          ...
          const jobState = require(path.resolve(
          ...
          for (const job of jobState.bulkImports) {
            ...
            newStatus.push(js);
            verboseLog(
              `Job with id ${color(jobid)} is in status : ${colorizeStatus(
                `${js.status}`
              )} [${js.message}]`,
              options.verbose,
              spinner
          ...
          Object.keys(result).forEach((key: string) => {
            console.log(
              `${result[key].length} job(s) in status ${colorizeStatus(key)}.`
            );
          });
        } catch (err) {
    ...
    .on("--help", () => {
      log("\n  Examples:\n");
      log(
        `    mc check-bulk \t displays job progress of ${color(
          "bulkimport"
        )} directory`
      );
      log(
