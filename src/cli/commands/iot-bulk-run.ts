...
export default (program: CommanderStatic) => {
  program
    ...
    .option(
      "-f, --force",
      "force generation of json files, file upload and creation of jobs"
    )
    ...
    .action(options => {
      (async () => {
        try {
          ...
          if (
            !jobstate.uploadFiles ||
            jobstate.uploadFiles.length === 0 ||
            options.force
          ) {
            ...
            verboseLog(
              `${color("skipping")} generation of json files..`,
              options.verbose,
              spinner
            );
            await sleep(500);
          ...
          if (options.start) {
            ...
            log(
              `Run time: ${(endDate.getTime() - startDate.getTime()) /
                1000} seconds`
            );
    ...
    .on("--help", () => {
      log("\n  Examples:\n");
      log(
        `    mc run-bulk runs the upload job from the  ${color(
          "bulkimport"
        )} directory`
      );
      log(
...
async function runTimeSeriesUpload(
  options: any,
  jobstate: jobState,
  spinner: ora.Ora
) {
  ...
  const auth = loadAuth();
  const tsClient = new TimeSeriesClient(
    auth.gateway,
  ...
  for (const file of jobstate.uploadFiles) {
    ...
    if (jobstate.timeSeriesFiles.indexOf(file.path) >= 0 && !options.force) {
      verboseLog(
        `${color(timeSeries.length)} records from ${formatDate(
          file.mintime
        )} to ${formatDate(file.maxtime)} were already posted`,
        options.verbose,
        spinner
      );
      await sleep(100);
    ...
    verboseLog(
      `posted ${color(timeSeries.length)} records from ${formatDate(
        file.mintime
      )} to ${formatDate(file.maxtime)} with pause of ${(pause / 1000).toFixed(
        2
      )}s between records`,
      options.verbose,
      spinner
...
async function runSimulationUpload(
  options: any,
  jobstate: jobState,
  spinner: ora.Ora
) {
...
async function createUploadJobs(
  jobstate: jobState,
  options: any,
  spinner: ora.Ora
) {
  ...
  for (const fileInfos of results as any) {
    const first =
      (_(fileInfos).first() as fileInfo) || throwError("no data in results");
    const data: TimeSeriesBulkModels.Data = {
      entity: first.entity,
      propertySetName: first.propertyset,
      timeseriesFiles: fileInfos.map(
        (x: { filepath: any; mintime: any; maxtime: any }) => {
  ...
  for (const bulkImport of jobstate.bulkImports) {
    ...
    verboseLog(
      `Job with ${color("" + job.jobId)} is in status : ${job.status} [${
        job.message
      }]`,
      options.verbose,
      spinner
  ...
  for (const entry of jobstate.uploadFiles) {
    ...
    if (entry.etag === undefined || options.force) {
      ...
      const fileInfo = await fileUploadClient.GetFiles(entry.entity, {
        filter: `name eq ${path.basename(
          entry.filepath
        )} and path eq ${path.dirname(entry.filepath)}/`
      });
      ...
      verboseLog(
        `The file  ${color(entry.filepath)} was already uploaded`,
        options.verbose,
        spinner
      );
      await sleep(500);
    ...
    verboseLog(
      `uploaded ${entry.filepath} with md5 checksum: ${result}`,
      options.verbose,
      spinner
    );
    const fileInfo = await fileUploadClient.GetFiles(
      `${jobstate.options.asset.assetId}`,
      {
        filter: `name eq ${path.basename(
          entry.filepath
...
async function createJsonFilesForUpload({
  ...
  for (const aspect of aspects) {
    ...
    for (const file of files) {
      ...
      await verboseLog(
        `reading file: ${options.dir}/csv/${aspect}/${color(file)}`,
        options.verbose,
        spinner
      );
      await csv()
        ...
        .subscribe(async json => {
          ...
          ({ mintime, maxtime } = determineMinAndMax(
            mintime,
            timestamp,
            maxtime
          ));
      ...
      verboseLog(
        `total record count in ${file}: ${color(recordCount.toString())}`,
        options.verbose,
        spinner
      );
    }
...
function writeDataAsJson({
  ...
  mintime ||
    maxtime ||
    throwError("the data is ivalid the timestamps are corrupted");
  const newFileName = `${aspect}_${mintime && mintime.toISOString()}`.replace(
    /[^a-z0-9]/gi,
    "_"
  );
  verboseLog(
    `writing ${options.dir}/json/${aspect}/${color(newFileName + ".json")}`,
    options.verbose
  );
...
function getFiles(
  options: any,
  aspect: string,
  csvorjson: "csv" | "json" = "csv"
) {
  verboseLog(
    `reading directory ${options.dir}/${csvorjson}/${color(aspect)}`,
    options.verbose
  );
  const files = fs
    .readdirSync(`${options.dir}/${csvorjson}/${aspect}`)
    .filter(x => {
...
function getAspectsFromDirNames(options: any): string[] {
  ...
  verboseLog(
    `aspect directories ${JSON.stringify(aspects)} in csv directory`,
    options.verbose
  );
  return aspects;
}
...
async function createOrReadAsset(options: any) {
  ...
  if (!asset.assetId) {
    ...
    verboseLog(
      `$asset ${color(asset.name)} with id ${color(asset.assetId)} created`,
      options.verbose
    );
  } else {
    verboseLog(
      `reading asset ${color(asset.name)} ${color(asset.assetId)}`,
      options.verbose
    );
    asset = await assetMgmt.GetAsset(asset.assetId);
    verboseLog(
      `asset ${color(asset.name)} ${color(
        asset.assetId
...
function checkRequiredParamaters(options: any) {
  ...
  !fs.existsSync(options.dir) &&
    throwError(`the directory ${color(options.dir)} doesn't exist!`);
