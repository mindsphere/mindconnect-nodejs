...
export default (program: CommanderStatic) => {
  program
    ...
    .option(
      "-w, --twintype <mode>",
      "twintype of asset [performance|simulation]"
    )
    ...
    .description(
      color("creates a template directory for timeseries (bulk) upload *")
    )
    .action(options => {
      (async () => {
        try {
          ...
          const auth = loadAuth();
          const { root, asset, aspects } = await getAssetAndAspects(
            auth,
            options
          );
          ...
          options.twintype === AssetManagementModels.TwinType.Performance &&
            options.offset < 7 &&
            console.log(
              `\n${yellow(
                "Warning!"
              )} The data has to be older then 7 days for bulk upload of performance assets.\n`
            );
        } catch (err) {
    ...
    .on("--help", () => {
      ...
      log(
        `\n\tuse --mode ${color(
          "performance"
        )} for standard data generation or --mode ${color(
          "simulation"
        )} for high frequency data generation `
      );
      log(
        `\tThe typeid must be derived from ${color(
          "core.basicdevice"
        )} and asset ${color("twintype")} must be ${color(
          "simulation"
  ...
  if (options.assetid) {
    ...
    if (embAspects._embedded && embAspects._embedded.aspects) {
      aspects = embAspects._embedded.aspects.filter(x => {
        return (
          x.category ===
          AssetManagementModels.AspectResource.CategoryEnum.Dynamic
        );
      });
    }
  }
...
function checkRequiredParamaters(options: any) {
  ...
  !options.typeid &&
    !options.assetid &&
    throwError("You have to specify either a typeid or assetid");

  !!options.typeid === !!options.assetid &&
    throwError("You can't specify typeid and assetid at the same time");
  verboseLog(
    `creating directory template for: ${color(
      options.typeid || options.assetid
    )}`,
    options.verbose
  );
  ...
  fs.existsSync(options.dir) &&
    throwError(`the directory ${color(options.dir)} already exists`);
...
async function generateCsv(
  ...
  verboseLog(
    `Generating ${options.size} entries for ${name}`,
    options.verbose,
    spinner
  );
...
function createAspectDirs(
  path: any,
  element: AssetManagementModels.AssetTypeResourceAspects,
  options: any
) {
...
function checkTwinTypeOfAsset(
  ...
  asset &&
    ...
    throwError(
      `You can't use the twintype mode ${color(options.twintype)} for ${
        asset.twinType
      } asset`
    );
}
