...
export default (program: CommanderStatic) => {
    program
        ...
        .option("-t, --status <status> [DOWNLOAD|INSTALL|ACTIVATE|CANCELED|FAILED]", "DOWNLOAD | INSTALL | ACTIVATE | CANCELED | FAILED")
...
async function listInstalls(sdk: MindSphereSdk, options: any) {
    ...
    do {
        iTaskPage = (await retry(options.retry, () =>
            appDeploymentClient.GetInstallationTasks(deviceid)
    ...
    // Print out the table
    console.log("Installation Tasks");
...
async function checkTermAndConditions(options: any, sdk: MindSphereSdk) {
    ...
    if (info.firstAccepted) {
        ...
        console.log(`No accepted terms and conditions for deviceid: ${deviceid} and realeaseid: ${realeaseid}.\n`);
    }
}
