...
describe("Agent Auth Rotation", () => {
  ...
  it.only("should be able to recover from a problem with key rotation.", async () => {
    ...
    const errorShouldOccur = await errorHelper(() =>
      (agent as any).RotateKey()
    );
  ...
  it.only("should be able to handle errors in key rotation", async () => {
    ...
    ((agent as any)._configuration
      .response as ClientIdentifier).client_secret_expires_at = Math.trunc(
