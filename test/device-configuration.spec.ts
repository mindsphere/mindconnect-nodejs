...
describe("[SDK] DeviceManagementClient.DeviceConfiguration", () => {
    ...
    it("should GET status of a configuration task, which should be canceled", async () => {
        ...
        (status as any).currentState.should.not.be.undefined;
        (status as any).currentState.should.not.be.null;
        (status as any).currentState.state.should.be.equal(DeviceConfigurationModels.ConfigurationStateInfo.StateEnum.CANCELED.toString());
    });
    ...
    async function deleteFiles() {
        ...
        do {
            files = (
                await deviceConfigurationClient.GetFiles(`/${tenant}/TEST`, 100)
    ...
    async function CancelAllGeneratedConfigurationTaks() {
        await sleep(2000);
        let tasks = null;
        let page = 0;
        do {
            tasks = (
                await deviceConfigurationClient.GetConfigurationTasks(deviceId)
            ) as DeviceConfigurationModels.PaginatedConfigurationTask;
            tasks.content = tasks.content || [];
            tasks.page = tasks.page || { totalPages: 0 };

            for (const _configTask of tasks.content || []) {
                const task = _configTask as DeviceConfigurationModels.ConfigurationTask;
                if (task.customData.name.toString().startsWith( "TEST_DEVICE_CONFIGURATION")) {
                    // Cancel task
                    const _config = {
                        state: DeviceConfigurationModels.Updatetask.StateEnum.CANCELED,
                            progress: 0,
                        message: "Configuring.",
                        details: {}
                    };
        ...
        } while (page++ < (tasks.page.totalPages || 0));
    }
});
