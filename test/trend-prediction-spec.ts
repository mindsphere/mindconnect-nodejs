...
describe("[SDK] TrendPredictionClient", () => {
  ...
  it("SDK should train the model and predict the results in two separate steps", async () => {
    ...
    Math.round(
      predictionResult[0].timeSeries![0]!["powerOutputSensor"]
    ).should.be.equal(-37); // f(x)=x*x -15x + 7 ;// https://www.wolframalpha.com/input/?i=x%5E2-15x%2B7%3B+x+%3D+4

    Math.round(
      predictionResult[0].timeSeries![1]!["powerOutputSensor"]
    ).should.be.equal(-7); // f(x)=x*x -15x + 7 ;// https://www.wolframalpha.com/input/?i=x%5E2-15x%2B7%3B+x+%3D+14

    Math.round(
      predictionResult[0].timeSeries![2]!["powerOutputSensor"]
    ).should.be.equal(83); // f(x)=x*x -15x + 7 ;// https://www.wolframalpha.com/input/?i=x%5E2-15x%2B7%3B+x+%3D+-4
  });
